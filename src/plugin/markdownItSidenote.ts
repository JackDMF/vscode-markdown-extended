...
const SN_TOKEN_CLOSE_CODE = SN_TOKEN_CLOSE.charCodeAt(0);

const MN_TOKEN_OPEN = '!';
const MN_TOKEN_OPEN_CODE = MN_TOKEN_OPEN.charCodeAt(0);
const MN_TOKEN_CLOSE = '!';
const MN_TOKEN_CLOSE_CODE = MN_TOKEN_CLOSE.charCodeAt(0);
...
function sidenoteTokenizer(state: any, silent: boolean): boolean {
    ...
    if (state.src.charCodeAt(state.pos) !== SN_TOKEN_OPEN_CODE && state.src.charCodeAt(state.pos) !== MN_TOKEN_OPEN_CODE) {
        return false;
    }
    ...
    if (start + 2 >= max ||
        (state.src.charCodeAt(start) !== SN_TOKEN_OPEN_CODE ||
            state.src.charCodeAt(start + 1) !== SN_TOKEN_OPEN_CODE) &&
        (state.src.charCodeAt(start) !== MN_TOKEN_OPEN_CODE ||
            state.src.charCodeAt(start + 1) !== MN_TOKEN_OPEN_CODE)) {
        return false;
    }

    if (state.src.charCodeAt(start) === SN_TOKEN_OPEN_CODE && state.src.charCodeAt(start + 1) === SN_TOKEN_OPEN_CODE) {
        // Sidenote
        // Find closing markers
        const endMarker = `${SN_TOKEN_CLOSE}${SN_TOKEN_CLOSE}`;
        ...
        tokenOpen.markup = `${SN_TOKEN_OPEN}${SN_TOKEN_OPEN}`;
        ...
        state.pos = endPos + 2;
    } else if (state.src.charCodeAt(start) === MN_TOKEN_OPEN_CODE && state.src.charCodeAt(start + 1) === MN_TOKEN_OPEN_CODE) {
        // Sidenote
        // Find closing markers
        const endMarker = `${MN_TOKEN_CLOSE}${MN_TOKEN_CLOSE}`;
        let endPos = state.src.indexOf(endMarker, start + 2);
        if (endPos === -1) return false;
        ...
        console.log("Found marginal note:", text, "with note:", note);

        // Direkt Token-Struktur erstellen, ohne Postprocessor

        // Öffnungs-Token für die Sidenote
        const tokenOpen = state.push('marginal_note_open', 'span', 1);
        tokenOpen.markup = `${SN_TOKEN_OPEN}${SN_TOKEN_OPEN}`;

        // Text-Token für den Anzeigetext
        const tokenText = state.push('marginal_note_text', '', 0);
        tokenText.content = text;

        // Öffnungs-Token für den Notizinhalt
        const tokenNoteOpen = state.push('marginal_note_content_open', 'span', 1);
        ...
        try {
            ...
            console.error("Error processing marginal note:", e);
            // Bei Fehler einfach Rohtext einfügen
            const fallbackText = state.push('text', '', 0);
            fallbackText.content = note;
        }
        ...
        // Schließende Tokens
        state.push('marginal_note_content_close', 'span', -1);
        state.push('marginal_note_close', 'span', -1);
