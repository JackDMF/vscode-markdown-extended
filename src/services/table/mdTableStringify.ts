...
export function stringifyMDTable(table: MDTable, compact?: boolean, padding?: number): string {
    padding = padding || 1;
    let rows = table.data.map((row, i) => table.indentation + stringifyRow(row, table.columnWidths, table.aligns, table.rowMergeFlags[i] || false, compact || false, padding));
    let Sep = table.indentation + stringifyHeaderSeperator(table, compact || false, padding);
